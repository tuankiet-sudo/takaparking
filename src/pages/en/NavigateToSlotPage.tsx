...
const elevators = [
    { name: "Zone A Elevator", betweenCols: [3, 4], spansRows: [2, 4], orientation: 'vertical' },
    { name: "Zone B Elevator", betweenRows: [5, 6], spansCols: [8, 10], orientation: 'horizontal' },
    { name: "Office Elevator", betweenRows: [1, 2], spansCols: [9, 11], orientation: 'horizontal' },
];
...
const NavigateToSlotPageEn = () => {
    ...
    const [pageTitle, setPageTitle] = useState('Parking Map');
    ...
    useEffect(() => {
        ...
        if (!savedVehicleData) {
            ...
            setPageTitle("Cannot find vehicle location");
            return;
        }

        const savedVehicle: { position: string } = JSON.parse(savedVehicleData);
        let parts = savedVehicle.position.match(/Column ([A-L])(\d{1,2})/);
        if (!parts) {
            parts = savedVehicle.position.match(/Cột ([A-L])(\d{1,2})/)
        }
        if (!parts) {
        ...
        setPageTitle(`Navigation to Column ${vehicleLocationString}`);
    ...
    const generateSpokenDirections = (path: Node[], targetLocation: string): string => {
        ...
        if (path.length < 2) {
            return `You are already at column ${targetLocation}.`;
        }

        const directions: string[] = [`Starting navigation to column ${targetLocation}.`];
        let straightCount = 0;
        ...
        for (let i = 0; i < path.length - 1; i++) {
            ...
            if (i < path.length - 2) {
                ...
                if (vector1.x !== vector2.x || vector1.y !== vector2.y) {
                    ...
                    directions.push(`Go straight ${straightCount} column.`);
                    ...
                    if (crossProduct > 0) {
                        directions.push("Then, turn left.");
                    } else if (crossProduct < 0) {
                        directions.push("Then, turn right.");
                    }
                }
            }
        }
        ...
        directions.push(`You have arrived. Your vehicle is next to the column ${targetLocation}.`);
    ...
    const handleSpeak = async () => {
        ...
        try {
            ...
            const response = await fetch('https://takaparking-be.vercel.app/api/ttsen', {
                method: 'POST',
    ...
    const draw = () => {
        ...
        ctx.fillText('Parking Area for Cars', carRectX + (carRectEndX - carRectX) / 2, carRectY + (carRectEndY - carRectY) / 2);
        ...
        ctx.font = 'italic 28px Arial';
        ctx.textAlign = 'center';
        ctx.fillText('Booking-only Area', bookingRectX + (bookingRectEndX - bookingRectX) / 2, bookingRectY + (bookingRectEndY - bookingRectY) / 2 + 60);
        ...
        ctx.fillText('ENTRANCE', entranceX + entranceWidth / 2, entranceY + entranceHeight / 2);
        ...
        <Box sx={{ bgcolor: '#f7f7f7', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Paper elevation={0} sx={{ p: 1.5, mx: 2, mt: 2, borderRadius: '12px', bgcolor: 'white' }}>
                <Stack direction="row" spacing={2} alignItems="center" justifyContent="center" flexWrap="wrap">
                    <LegendItem color="#E53935" text="Path to your vehicle" />
                    <LegendItem color="#FFC107" text="Path to exit" />
                    <LegendItem color="#1976d2" text="Vehicle Location" isBox />
                </Stack>
            </Paper>
...
export default NavigateToSlotPageEn;