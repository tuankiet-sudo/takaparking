            ...
            <Box sx={{ p: 2 }}>
                <Paper elevation={0} sx={{ p: 3, borderRadius: '16px', bgcolor: 'white' }}>
                    ...
                    <Stack alignItems="center" spacing={1} sx={{ mb: 2 }}>
                        ...
                        <Typography variant="h6" sx={{ fontWeight: 'bold' }}>Payment Successful</Typography>
                        <Typography variant="body2" sx={{ color: 'text.secondary' }}>{transaction.paymentTimestamp}</Typography>
                    </Stack>

                    {/* --- Transaction Details --- */}
                    <Stack divider={<Divider flexItem />}>
                        <DetailRow icon={<ReceiptLongIcon color="action" />} label="Transaction ID" value={transaction.id} />
                        <DetailRow icon={<ConfirmationNumberIcon color="action" />} label="License Plate" value={transaction.licensePlate} />
                        <DetailRow icon={<AccessTimeIcon color="action" />} label="Payment Time" value={transaction.paymentTimestamp} />
                        <DetailRow icon={<LocationOnIcon color="action" />} label="Locat" value={transaction.location} />
                    </Stack>

                    {/* --- Exit Warning --- */}
                    <Typography sx={{ color: 'error.main', textAlign: 'center', mt: 3, fontWeight: 'medium' }}>
                        Please retrieve your vehicle before {formatTime(exitByTime)} on {formatDateTime(exitByTime).split(' | ')[0]} to avoid additional parking fees
                    </Typography>
                </Paper>
...
export default PaymentSuccessPageEn;
