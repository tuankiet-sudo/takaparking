...
const BookingPageEn = () => {
    ...
    useEffect(() => {
        ...
        const fetchSlots = async () => {
            ...
            try {
                ...
                console.error(error);
                setSnackbarMessage('Failed to fetch slot data from server.');
    ...
    const handleSlotClick = (column: string, slotNumber: number) => {
        ...
        if (selectedSlots.length >= MAX_BOOKABLE_SLOTS) {
            setSnackbarMessage(`You can only book up to ${MAX_BOOKABLE_SLOTS} slots.`);
            setSnackbarOpen(true);
            return;
        }
    ...
    const handleBooking = async () => {
        if (selectedSlots.length === 0 || !userSelectedTime) {
            alert('Please choose slot(s) and booking time.');
            return;
        }
        ...
        try {
            ...
            navigate('/en/parking/book-slot/success', {
                state: {
    ...
    return (
        <LocalizationProvider dateAdapter={AdapterDayjs}>
            <Box sx={{ bgcolor: '#f7f7f7', minHeight: '100vh', pb: 4 }}>
                ...
                <Box sx={{ p: 2 }}>
                    {isLoading ? (
                        <Stack alignItems="center" mt={4}><CircularProgress /><Typography sx={{ mt: 2 }}>Loading Empty Slots...</Typography></Stack>
                    ) : (
                        <>
                            <Typography variant="body1" sx={{ textAlign: 'center', color: 'text.secondary', mb: 2 }}>
                                Please select the time and location you want to book at Basement B3 - Takashimaya
                            </Typography>
                            <Paper elevation={0} sx={{ p: 2, borderRadius: '16px', mb: 2 }}>
                                <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 2 }}>Booking Time</Typography>
                                <Stack spacing={2}>
                                    <DatePicker 
                                        label="Day to book" 
                                        value={selectedDate} 
                                    ...
                                    />
                                    <Box>
                                        <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 1 }}>Arrival Time</Typography>
                                        <Stack direction="row" spacing={2}>
                                            <FormControl fullWidth>
                                                <InputLabel>Hour</InputLabel>
                                                <Select value={selectedHour} label="Hour" onChange={(e) => setSelectedHour(e.target.value)}>
                                                    {availableHours.map(h => <MenuItem key={h} value={h}>{h}</MenuItem>)}
                                                </Select>
                                            </FormControl>
                                            <FormControl fullWidth>
                                                <InputLabel>Minute</InputLabel>
                                                <Select value={selectedMinute} label="Minute" onChange={(e) => setSelectedMinute(e.target.value)}>
                            ...
                            <Paper elevation={0} sx={{ p: 2, borderRadius: '16px', mb: 2 }}>
                                <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 2 }}>Location</Typography>
                                ...
                                {(vehicleType === 'motorbike' || vehicleType === 'car') && (
                                     <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
                                        <FormControl fullWidth>
                                            ...
                                            <Select value={isElectric} label="Vehicle Type" onChange={(e) => setIsElectric(e.target.value as 'gas' | 'electric')}>
                                                <MenuItem value="gas">Gas</MenuItem>
                                                <MenuItem value="electric">Electric</MenuItem>
                                            </Select>
                                        </FormControl>
                                        <FormControl fullWidth>
                                            <InputLabel>Charge Needs</InputLabel>
                                            <Select value={willCharge} label="Charge Needs" onChange={(e) => setWillCharge(e.target.value as 'yes' | 'no')} disabled={isElectric === 'gas'}>
                            ...
                            <Paper elevation={0} sx={{ p: 2, borderRadius: '16px', mb: 3 }}>
                                <Stack direction="column" spacing={2} justifyContent="center" flexWrap="wrap">
                                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 1 }}>Notes</Typography>
                                        <LegendItem color="#66BB6A" text="You can book freely with no time restrictions" />
                                        <LegendItem color="#FFA726" text="You can book freely but with time limits due to a later reservation" />
                                        <LegendItem color="#E53935" text="You cannot book due to a nearby reservation in your selected time range" />
                                </Stack>
                            </Paper>
                ...
                <Modal open={warningModalOpen} onClose={handleCancelYellowSlot} closeAfterTransition sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Fade in={warningModalOpen}>
                        <Paper sx={{ p: 3, borderRadius: '16px', textAlign: 'center', maxWidth: '90%', width: '400px' }}>
                            ...
                            <Typography variant="h5" sx={{ fontWeight: 'bold', mb: 2 }}>Notice</Typography>
                            <Typography sx={{ color: 'text.secondary', mb: 3, textAlign: 'left' }}>This slot is booked for the time <strong>{warningSlotInfo?.bookedTime}</strong>, you will need to take out your vehicle before that time.<br/><br/>Click <strong>"Confirm"</strong> if you confirm to book this slot.<br/><br/><Typography component="span" sx={{ color: 'error.main', fontWeight: 'bold' }}>In case of non-compliance, your vehicle will be moved from the booked spot and a penalty fee of 100,000 VND will be applied.</Typography></Typography><Stack direction="row" spacing={2}><Button variant="outlined" fullWidth onClick={handleCancelYellowSlot}>Cancel</Button><Button variant="contained" fullWidth onClick={handleConfirmYellowSlot} sx={{ backgroundColor: '#E53935', '&:hover': { backgroundColor: '#C62828' } }}>Confirm</Button></Stack></Paper></Fade>
                </Modal>
