...
const PaymentConfirmationPageEn = () => {
        ...
        <Box sx={{ bgcolor: '#f7f7f7', pb: 4 }}>
            ...
            <Box sx={{ p: 2 }}>
                ...
                <Paper elevation={0} sx={{ p: 2, borderRadius: '16px', bgcolor: 'white' }}>
                    ...
                    <Stack spacing={1}>
                        <Stack direction="row" alignItems="center" spacing={1.5}>
                            ...
                            <Typography>License Plate: <strong>{transaction.licensePlate}</strong></Typography>
                        </Stack>
                        <Stack direction="row" alignItems="center" spacing={1.5}>
                            <AccessTimeIcon sx={{ color: 'text.secondary' }} />
                            <Typography>Check-in time: <strong>{transaction.timeIn}</strong></Typography>
                        </Stack>
                        <Stack direction="row" alignItems="center" spacing={1.5}>
                            <LocationOnIcon sx={{ color: 'text.secondary' }} />
                            <Typography>Location: <strong>{transaction.location}</strong></Typography>
                        </Stack>
                    </Stack>
                </Paper>
                ...
                <Paper elevation={0} sx={{ p: 2, mt: 2, borderRadius: '16px', bgcolor: 'white' }}>
                    <Typography sx={{ fontWeight: 'bold', mb: 1 }}>Invoice / Discount Code</Typography>
                    <Button variant="outlined" fullWidth startIcon={<CameraAltIcon />} sx={{ justifyContent: 'flex-start', p: 1.5, color: 'text.secondary', borderColor: '#e0e0e0' }}>
                        Scan invoice code / voucher
                    </Button>
                </Paper>

                {/* --- Fee Details --- */}
                <Paper elevation={0} sx={{ p: 2, mt: 2, borderRadius: '16px', bgcolor: 'white' }}>
                    <Typography sx={{ fontWeight: 'bold', mb: 1 }}>Parking Fee Details</Typography>
                    <Stack spacing={1} divider={<Divider flexItem />}>
                        <DetailRow label="Fee" value={transaction.fee} />
                        <DetailRow label="Parking duration" value={transaction.duration} />
                        <DetailRow label="Discount" value={transaction.discount} />
                        <DetailRow label="Total" value={transaction.total} valueColor="red" />
                    </Stack>
                </Paper>

                {/* --- QR Code and Countdown Section --- */}
                <Paper elevation={0} sx={{ p: 2, mt: 2, borderRadius: '16px', bgcolor: 'white', textAlign: 'center' }}>
                    <Typography sx={{ fontWeight: 'bold', mb: 1 }}>Please scan the QR code to make a payment</Typography>
...
export default PaymentConfirmationPageEn;
