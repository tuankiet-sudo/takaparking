...
const viTriSubOptions = [
    { id: 'save-slot', name: 'LƯU VỊ TRÍ', icon: <SaveIcon sx={{color: 'white', fontSize: 32}}/>, path: '/parking/save-slot' },
    { id: 'find-vehicle', name: 'TÌM XE', icon: <FindInPageIcon sx={{color: 'white', fontSize: 32}}/>, path: '/parking/find-vehicle' },
];
    ...
    <Box>
      ...
      <Grid container spacing={2}>
                {parkingOptions.map((cat) => {
                    ...
                    const itemContent = (
                        <Box sx={{ textAlign: 'center', cursor: 'pointer' }} onClick={cat.name === 'VỊ TRÍ XE' ? handleOpen : undefined}>
                            <Box sx={{
