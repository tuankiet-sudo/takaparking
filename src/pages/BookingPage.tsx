...
import {
    ...
    FormControl, InputLabel, Select, MenuItem, Modal, Fade, Snackbar
} from '@mui/material';
...
const BookingPage = () => {
    ...
    const getSlotStatus = (column: string, slotNumber: number) => {
        ...
        if (!booking || !userSelectedTime) return 'available';
        ...
        if (bookedTime.isBefore(userSelectedTime)) return 'available';
        const minutesDifference = bookedTime.diff(userSelectedTime, 'minute');
        if (minutesDifference <= 60) return 'booked_before';
    ...
    // MODIFIED: Navigate to the new confirmation page
    const handleBooking = () => {
        if (selectedSlots.length > 0) {
            // In a real app, you would pass the booking data here
                    ...
                    <Fade in={warningModalOpen}>
                        <Paper sx={{ p: 3, borderRadius: '16px', textAlign: 'center', maxWidth: '90%', width: '400px' }}>
                            ...
                            <Typography sx={{ color: 'text.secondary', mb: 3, textAlign: 'left' }}>
                                Vị trí này đã được đặt trước vào lúc <strong>{warningSlotInfo?.bookedTime}</strong>, bạn cần lấy xe trước khung giờ đó.<br/><br/>
                                Hãy nhấn <strong>xác nhận</strong> nếu bạn đồng ý.<br/><br/>
